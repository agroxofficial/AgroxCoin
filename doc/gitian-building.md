...
*Setup instructions for a gitian build of AGROX using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the AGROX
...
Clone the git repositories for agrox and gitian and then checkout the agrox version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the agrox repository
...
To build AGROX (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the agrox repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/agrox/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/agrox/agrox
...
For example:
```bash
URL=https://github.com/konez2k/agrox.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit agrox=${COMMIT} --url agrox=${URL} ../agrox/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit agrox=${COMMIT} --url agrox=${URL} ../agrox/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit agrox=${COMMIT} --url agrox=${URL} ../agrox/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/agrox-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/agrox-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/agrox-build.assert
```
...
[agrox/gitian.sigs](https://github.com/agrox/gitian.sigs/) repository, or if that's not possible to create a pull
request.
